...
-- -----------------------------------------------------------------------------
-- Optional hybrid blockchain anchoring extension
-- -----------------------------------------------------------------------------
-- GoalOS Signoff Hybrid: blockchain anchor tracking.
-- This table mirrors public on-chain anchor status while keeping private receipt contents off-chain.

create table if not exists public.receipt_anchors (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references public.projects(id) on delete cascade,
  receipt_id uuid not null references public.receipts(id) on delete cascade,
  network text not null check (network in ('disabled','sepolia','ethereum-mainnet')),
  ...
  block_number bigint,
  block_hash text,
  status text not null check (status in ('queued','submitted','confirmed','failed','revoked')) default 'queued',
  error_message text,
  anchored_by uuid references auth.users(id),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique(receipt_id, chain_id, contract_address)
);

alter table public.receipt_anchors enable row level security;

create policy "receipt anchors are visible to project participants"
  on public.receipt_anchors for select
  using (public.is_project_member(project_id, auth.uid()));

create policy "receipt anchors are inserted by service role only"
  on public.receipt_anchors for insert
  with check (false);

create policy "receipt anchors are updated by service role only"
  on public.receipt_anchors for update
  using (false)
  with check (false);

create index if not exists receipt_anchors_project_idx on public.receipt_anchors(project_id);
create index if not exists receipt_anchors_receipt_idx on public.receipt_anchors(receipt_id);
create index if not exists receipt_anchors_tx_idx on public.receipt_anchors(transaction_hash);
